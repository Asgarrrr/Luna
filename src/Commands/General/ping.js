...
// —— Components to check
const servers  = [ "Europe", "Hong Kong", "India", "Japan", "Singapore",
                  "South Africa", "South Korea", "Sydney", "US Central",
                  "US East", "US South", "US West" ],

      services = [ "CloudFlare", "Media Proxy", "Tax Calculation Service",
                   "Push Notifications", "Search", "Voice", "Third-party" ];


function clear(from, into) {

    from = from.components
        .filter(element => into.includes(element.name))
        .map(x => [x.name, x.status === "operational" ? "✔" : "✗"]);

    return new Array(Math.ceil(from.length / 2))
        .fill()
        .map(_ => from.splice(0, 2))
        .map(_ => {

            return (_.length > 1)
                ? `${_[0][0].substring(0, 11).padStart(11)} │ ${_[0][1]} : ${_[1][1]} │ ${_[1][0].substring(0, 11)}`
                : `${_[0][0].substring(0, 11).padStart(11)} │ ${_[0][1]} :`
...
class Ping extends Command {
    ...
    async run(message) {
        ...
        // —— Retrieve the language information for this command
              lang   = client.language.get(message.guild.local).ping(Date.now(), message),
        ...
        try {
            ...
            const statuspage  = await (await fetch("https://srhpyqt94yxb.statuspage.io/api/v2/summary.json")).json();

            embed.fields.push([{
                    name  : "Services",
                    value : `\`\`\`${clear(statuspage, services)}\`\`\``
                },
                {
                    name  : "Servers",
                    value : `\`\`\`${clear(statuspage, servers)}\`\`\``
                }
            ])
