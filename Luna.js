...
function error(err) {
    console.error(chalk.bold.red(timestamp() + " × 🐛", error));
}
...
fs.readdir("./events/", (err, files) => {                                        // Imports all event files
    ...
    files.forEach(file => {                                                       // For each file found in the events folder
        const event = require(`./event/${file}`);                                // Include the file to be able to operate on it
...
client.on("message", async message => {                                         // Upon receipt of a new message
    ...
    if (!message.member.roles.some(role => role.name === Commande.privilèges)) {// Command Privilege
